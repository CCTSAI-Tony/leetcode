...
# 思路: sliding window, 轉換成 Find the smallest subarray sum of length len(cardPoints) - k
class Solution:
    def maxScore(self, cardPoints: List[int], k: int) -> int:
        size = len(cardPoints) - k
        min_sum = current_sum = sum(cardPoints[:size])
        for i in range(size, len(cardPoints)):
            current_sum += cardPoints[i] - cardPoints[i-size]
