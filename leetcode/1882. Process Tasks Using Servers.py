...
# 刷題用這個, time complexity O(nlogm), n = len(tasks), m = len(servers)
# 思路: 使用heap, 建立 2 heap 來做priority queue, sorting order 非常重要
import heapq
class Solution:
    def assignTasks(self, servers: List[int], tasks: List[int]) -> List[int]:
        ...
        for j, task in enumerate(tasks):
            while h2 and h2[0][0] <= j or not h1:
                time, weight, i = heapq.heappop(h2) # 相同時間 free task pop 順序按照 weight, index
                heapq.heappush(h1, [weight, i, time])  
            weight, i, time = heapq.heappop(h1)
            res.append(i)
            heapq.heappush(h2, [max(time,j)+task, weight, i])  # At second j, the jth task is inserted into the queue => max(time,j)
        return res


# 重寫第二次, time complexity O(nlogm), n = len(tasks), m = len(servers)
import heapq
class Solution:
    def assignTasks(self, servers: List[int], tasks: List[int]) -> List[int]:
        ans = []
        ...
        for j, task in enumerate(tasks):
            ...
            weight, i, time = heapq.heappop(h1)
            ans.append(i)
            heapq.heappush(h2, (max(j, time) + task, weight, i))
        return ans
