...
# time complexity O(len(nums) * sum(nums)), dp bottom up
# 思路: 只要能判斷有無subset sums up to sum(nums) / 2, 就能立刻判斷nums 可以平分subsets
...
class Solution:
    def canPartition(nums):
        ...
        for x in nums: #代表i iteration
            dp = [dp[s] or (s >= x and dp[s-x]) for s in range(target+1)] #s 代表sum up to 的target, 裡面的dp[s-x] 還是存在於上一次iteration生成的dp
            if dp[target]: 
                return True
        return False

# 自己重寫, time complexity O(len(nums) * sum(nums)), 1348ms
# dp[i][j] is a boolean that indicates whether we can pick some of the elements from nums[:i+1] to sum up to a specific value j.
class Solution:
...
class Solution:
    ...
    def dfs(self, i, target, memo, nums, n):
        if target not in memo:
            ...
            if target > 0:
                for j in range(i, n):
                    if self.dfs(j+1, target - nums[j], memo, nums, n): #j+1, 避免重複選到一樣元素
                        memo[target] = True
                        break
        return memo[target]
